// Super-Admin AI console (no auth for now): two sections — (1) API providers
// (links/keys, test, manual models) and (2) the complete agents list, where
// picking a provider lists its models ALPHABETICALLY and each agent has a
// Load button + status.
...
export function AdminAiConfig({ v }: { v: Record<string, any> }) {
  ...
  async function onTestProvider(id: string) {
    ...
    try {
      ...
      setMsg({ k: 'err', t: L('خطا در تست.', 'Test error.') })
    }
  }
    ...
    <div>
      ...
      <div style={css(CARD)}>
        <div style={css('display:flex;align-items:center;gap:8px;margin-bottom:4px')}>
          ...
          <div style={css('font-weight:800;font-size:15px')}>{L('۱ · سرویس‌دهنده‌های API', '1 · API providers')}</div>
          <div style={css('flex:1')}></div>
          <button
            onClick={addProvider}
            style={css('height:32px;padding:0 14px;background:var(--card2);border:1px dashed var(--bd2);border-radius:9px;color:var(--sub);font-family:inherit;font-size:12px;font-weight:700;cursor:pointer')}
          >
            {L('+ سرویس‌دهنده', '+ Provider')}
          </button>
        </div>
        <div style={css('font-size:11.5px;color:var(--mut);margin-bottom:14px')}>
          {L('لینک API و کلید هر سرویس‌دهنده اینجاست. کلیدها فقط روی سرور می‌مانند.', 'API URL and key per provider. Keys stay server-side.')}
        </div>
      ...
      <div style={css(CARD)}>
        <div style={css('display:flex;align-items:center;gap:8px;margin-bottom:4px')}>
          ...
          <div style={css('font-weight:800;font-size:15px')}>{L('۲ · ایجنت‌های پروژه', '2 · Project agents')}</div>
